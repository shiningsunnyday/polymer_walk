...
def diffuse(graph, log_folder, **diff_args):
    ...
    W = nn.Parameter(torch.zeros((len(G), len(G)),dtype=torch.float64))
    A = torch.as_tensor(nx.adjacency_matrix(G).toarray())
    loss_func = nn.MSELoss()   
    opt = torch.optim.SGD([W], lr=diff_args['alpha'])
    history = []
    T = 10
    for i in range(diff_args['num_epochs']):
        ...
        for t in range(T):
            ...
            L = D-W # (N, N)
            X = torch.as_tensor(graph.get_state(not diff_args['combine_walks'])) # (batch, N)
            graph.step()
            Y = torch.as_tensor(graph.get_state(not diff_args['combine_walks'])) # (batch, N)
            opt.zero_grad()     
            loss = loss_func(X+X@L.T, Y) 
            loss.backward()
            opt.step()
            print(f"epoch {i} time {t} loss: {loss}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--alpha', dest='diffusion_alpha', default=1e-2, type=float)
