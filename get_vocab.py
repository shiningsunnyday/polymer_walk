...
def break_bonds(mol, bonds, red_grps):
    ...
    for bs, (red_grp1, red_grp2) in zip(bonds, red_grps):
        ...
        assert min(indices) != -1, f"segmentation is not valid at {bs}!"            
...
def draw_grid(pics, titles, fpath=""):
    ...
    f, axes = plt.subplots(grid_dim, (len(pics)-1)//grid_dim+1, figsize=(50,50))
    ...
    for i in range(len(pics)):
        ...
        axes[i//grid_dim][i%grid_dim].title.set_fontsize(10)
...
def main(args):
    ...
    if args.props_file:
        ...
        elif 'arfb' in args.data_file:
            props = []
            for l in lines:
                props.append(f"re_pot,sol={l.split(',')[-2:]}")       
        elif 'amorphous' in args.data_file:
            props = []
            for l in lines:
                props.append(f"thermal_c={l.split(',')[-1]}")               
        elif 'cholecystokinin_inhibition_115' in args.data_file:
            props = []
            for l in lines:
                props.append(f"pchembl={l.split(',')[-1]}")                               
        elif 'lohc' in args.data_file:
            props = []
            for l in lines:
                props.append(f"delta_h={l.split(',')[1]},wt_h2={l.split(',')[-1]}")
        else:
...
def seg_groups(args):
    ...
    for i, line in enumerate(data):        
        ...
        if not G.edges():
            ...
            G.add_edge(root, root)
        # nx.write_edgelist(G, os.path.join(walk_dir, f"walk_{i}.edgelist"))        
    if args.seg_json_out_file:
        breakpoint()
        json.dump(segs, open(args.seg_json_out_file, 'w+'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--group_dir', help='where to store outputs of segmentation')
    parser.add_argument('--seg_dir', help='where to segment molecules (usually same as --group_dir)')
    parser.add_argument('--do_seg', action='store_true', help='whether to segment molecule into groups; if not, just visualize molecules')
    parser.add_argument('--seg_file')
    parser.add_argument('--data_file', help='file of smiles strings')
